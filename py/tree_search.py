...
def max_likelihood_tree_search(
    ...
    while True:
        ...
        params, history = optimize_likelihood(
            T, raw_params, m, dt, learning_rate, optimization_loop_steps, grad_clip_norm
        )
        ml = history[-1]
        ...
        for nT in nni_neighborhood(T):
            ...
            branch_lengths = jnp.array(
                [T.get_edge_data(u, v)["weight"] for u, v in edge_order(T)],
                dtype=jnp.float32,
            )
            params["branch_lengths"] = branch_lengths  # type: ignore
            this_likel = neg_log_likelihood_from_raw_params(nT, params, m, dt)
    ...
    params, history = optimize_likelihood(
        T, params, m, dt, learning_rate, final_steps, grad_clip_norm
    )
...
def test_tree_search():
    ...
    _, raw_opt, ml = max_likelihood_tree_search(
        T, raw, 3, 0.5, 1e-2, 100, 1000, 1.0, 1e-5, 3
    )
